defmodule StubAliasTest do
  ...
  test "configured" do
    ...
    assert "alias(Stubs.MyModule, as: MyModule)" == Macro.expand(@stub, __ENV__) |> Macro.to_string
  end

  test "configured name mismatch" do
    Application.put_env(:stub_alias, :"MyModule", Stubs.StubModule)
    assert "alias(Stubs.StubModule, as: MyModule)" == Macro.expand(@stub, __ENV__) |> Macro.to_string
  end

  @stub_nested quote(do: stub_alias MyModule.MyThing)
  test "nested unconfigured" do
    assert "alias(MyModule.MyThing, as: MyThing)" == Macro.expand(@stub_nested, __ENV__) |> Macro.to_string
  end

  test "nested configured" do
    Application.put_env(:stub_alias, :"MyModule.MyThing", Stubs.MyModule.MyThing)
    assert "alias(Stubs.MyModule.MyThing, as: MyThing)" == Macro.expand(@stub_nested, __ENV__) |> Macro.to_string
  end
  ...
  test "multi unconfigured" do
    assert """
    (
      alias(MyModule.Foo, as: Foo)
      alias(MyModule.Bar, as: Bar)
    )
  ...
  test "multi configured" do
    ...
    assert """
    (
      alias(Stubs.MyModule.Foo, as: Foo)
      alias(Stubs.MyModule.Bar, as: Bar)
    )
    """ |> String.rstrip  == Macro.expand(@stub_multi, __ENV__) |> Macro.to_string
  end
end
